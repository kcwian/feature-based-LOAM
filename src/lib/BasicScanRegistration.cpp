...
void BasicScanRegistration::processScanlines(const Time& scanTime, std::vector<pcl::PointCloud<pcl::PointXYZI>> const& laserCloudScans)
{
  ...
  //  std::cout << "Scan line size: " << laserCloudScans.size() << std::endl; // 16 for velodyne
   // std::cout << "Scan line size: " << laserCloudScans[i].size() << std::endl; // Varies from 1200 - 1900
  }

  // ----- Velodyne - not working

  // if (_laserDepthImage.empty())
  //   _laserDepthImage = cv::Mat(16, 400, CV_32FC1, cv::Scalar::all(0));
  // else
  //   _laserDepthImage = cv::Mat::zeros(_laserDepthImage.size(), CV_32FC1);
    
  // // Creates Image from laser Scan:
  
  // for (int i = 0; i < laserCloudScans.size(); i++)
  // {
  //   for (int j = 0; j < laserCloudScans[i].size(); j++)
  //     if(i <= 16 && j < 400)
  //     _laserDepthImage.at<float>(i, j) = sqrt(laserCloudScans[i][j].z * laserCloudScans[i][j].z  + laserCloudScans[i][j].y * laserCloudScans[i][j].y + laserCloudScans[i][j].x * laserCloudScans[i][j].x ) * 10;

  // }

//   static int n = 1;
//   //_laserDepthImage.convertTo(_laserDepthImage,CV_8UC1);
//   std::string path = ros::package::getPath("loam_velodyne") + "/scans_img/img" + std::to_string(n++) + ".png";
...
void BasicScanRegistration::extractFeatures(const uint16_t& beginIdx)
{
  ...
  for (size_t i = beginIdx; i < nScans; i++) {
    ...
    for (int j = scanStartIdx; j <= scanEndIdx; j++) {
      _laserCloud[j].processScanlines = i + _scanPeriod * (j - scanStartIdx) / scanSize;
    }*/
